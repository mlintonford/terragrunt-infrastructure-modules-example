# ---------------------------------------------------------------------------------------------------------------------
# A SIMPLE EXAMPLE OF HOW TO DEPLOY OCTOPUSDEPLOY
# This is an example of how to use Terraform to deploy an octpus deploy server. To keep the example simple, we deploy a vanilla Ubuntu AMI across the ASG and run a
...
resource "aws_autoscaling_group" "webserver_example" {
  ...
  vpc_zone_identifier  = ["subnet-67b9520f"]
...
resource "aws_launch_configuration" "webserver_example" {
  ...
  user_data = <<-EOF
              Install-Module -Name OctopusDSC -RequiredVersion 4.0.250
